...
| `copilot/domain`      | `@ortha-cms/copilot-domain`      | The copilot's framework-free core: the `ModelProvider` port, the tool contracts (`ToolSpec`, `COPILOT_TOOL_PROVIDER`), the capability profile, the run event vocabulary, and the untrusted-content fence. Imports no framework and no vendor SDK.                 |
| `copilot/server`      | `@ortha-cms/copilot-server`      | **Plugin.** `CopilotPlugin({ providers, resolve, config })` — the model seam plus the chat slice: the SSE run route, the bounded run engine, the capability profile, the model catalogue, and the three transcript tables it owns and migrates.                   |
| `copilot/admin`       | `@ortha-cms/copilot-admin`       | **Plugin.** The chat panel — a **non-modal docked window** (bottom-right, minimize/expand) opened from a floating corner button, the sidebar footer or `⌘J`, with a `fetch`-based SSE transport, tool-step disclosure and a per-turn model picker. Contributes no routes.                  |
