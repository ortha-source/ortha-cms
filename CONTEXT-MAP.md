...
| `content/graphql`            | `@ortha-cms/content-graphql`            | **Plugin.** The public content API over GraphQL (`POST /api/v1/graphql`, `GET` for SDL). An adapter over content-server's `public-api/` — same tokens, guards, scopes and visibility rules; the schema is built per workspace grant set (ADR-0007).                                                                                             |
...
| `copilot/domain`             | `@ortha-cms/copilot-domain`             | The copilot's framework-free core: the `ModelProvider` port, the normalised stream events, and the supported-capability baseline. Imports no framework and no vendor SDK.                                                                                                                                                                       |
| `copilot/server`             | `@ortha-cms/copilot-server`             | **Plugin.** `CopilotPlugin({ providers, resolve, config })` — binds the model registry + resolver + config. Phase 0: no routes, no engine, no tables.                                                                                                                                                                                           |
| `copilot/admin`              | `@ortha-cms/copilot-admin`              | **Plugin.** The chat panel's home. Phase 0: registered, contributes nothing yet.                                                                                                                                                                                                                                                                |
