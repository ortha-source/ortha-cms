...
| `mcp/server`                 | `@ortha-cms/mcp-server`                 | **Plugin.** The Model Context Protocol front door (`POST /api/v1/mcp`) **and** the shared, transport-neutral agent **tool registry**. Owns no tools — `content/server` contributes the content CRUD set; the copilot's tool loop will consume the same registry in-process. Off unless `MCP_ENABLED=true`.                                        |
