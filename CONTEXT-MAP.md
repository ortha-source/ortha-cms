...
| `copilot/provider-openai-compatible` | `@ortha-cms/copilot-provider-openai-compatible` | Adapter for the OpenAI wire format with a configurable `baseUrl` — Ollama, vLLM, llama.cpp, LM Studio, LiteLLM, OpenRouter, Azure, OpenAI. Makes a local install a setting, not a fork.            |
