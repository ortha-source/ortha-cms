...
| `content/graphql`            | `@ortha-cms/content-graphql`            | **Plugin.** The public content API over GraphQL (`POST /api/v1/graphql`, `GET` for SDL). An adapter over content-server's `public-api/` — same tokens, guards, scopes and visibility rules; the schema is built per workspace grant set (ADR-0006).                                                                                             |
