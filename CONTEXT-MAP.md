...
| `copilot/admin`       | `@ortha-cms/copilot-admin`       | **Plugin.** The chat panel — a right-hand sheet opened from the sidebar footer or `⌘J`, with a `fetch`-based SSE transport, tool-step disclosure and a per-turn model picker. Contributes no routes.                                                             |
