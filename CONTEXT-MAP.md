...
| `copilot/admin`              | `@ortha-cms/copilot-admin`              | **Plugin.** The chat panel — a **non-modal docked window** (bottom-right, minimize/expand) opened from a floating corner button, the sidebar footer or `⌘J`, with a `fetch`-based SSE transport, tool-step disclosure, a per-turn model picker, and the proposal card that decides a change. Contributes the workspace's Ortha AI settings route. |
