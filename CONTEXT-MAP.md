...
| `copilot/server`             | `@ortha-cms/copilot-server`             | **Plugin.** `CopilotPlugin({ providers, resolve, config })` — the model seam plus the chat slice: the SSE run route, the bounded run engine, the capability profile, the model catalogue, propose-then-apply with its accept boundary and per-workspace auto-apply policy, and the five tables it owns and migrates.                              |
