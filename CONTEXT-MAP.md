...
| `wysiwyg/admin`       | `@ortha-cms/wysiwyg-admin`       | **Plugin.** Rich-text editing: owns the control every `richtext` field renders — a content preview that expands into a TipTap editor filling the record's work area (callouts, tables, columns). Fills content's `ENTRY_FIELD_CONTROL_SLOT`; no routes, no server side. |
