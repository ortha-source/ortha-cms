...
- **EC-09 — `timeoutMs` shorter than the model's first token.** `❌ NONE`
  ...
  names it to the _user_ — see 🐞 BUG-copilot-provider-openai-01.
...
- **EC-39 — `config.capabilities` mutated after construction.** `❌ NONE`
  **Not** snapshotted — `resolveCapabilities(config, model)` reads
  `config.capabilities` at call time (`config.ts:57-66`), so a mutation _does_
...
- **♿ A11Y-copilot-provider-openai-04 — 2.2.1 Timing Adjustable.** WCAG
  **2.2.1 (A)** · 508 **E205.4** · Verdict: **Not Applicable (this unit)**
  `DEFAULT_TIMEOUT_MS` (120 000) bounds a _network_ request, not a human
...
names. The 500-character slice is a deliberate bound on _length_, not on
_content_. Note how carefully the same problem is handled one layer away:
...
the copilot into a _read oracle_ for anything the server can reach, because the
...
and OpenAI itself all speak" this format (AGENTS.md) — and the _one_ member of
...
this adapter exists precisely to _not_ hard-code assumptions about. When it does
...
edit-compile cycle — but the _plural_ change is load-bearing (it is what lets one
...
- **Does this package import a vendor SDK?** No. `package.json` declares only
  ...
  strongest form.
- **Can a _caller_ influence `baseUrl`, `headers` or `apiKey`?** No. None is on
