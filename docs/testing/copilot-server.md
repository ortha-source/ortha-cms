...
- **Any write.** A `propose` tool returns a `ProposalDraft`; the change is
  ...
  (`decide-proposal.service.ts:122-130`), which calls the _ordinary_ use-case.
...
- **The proposal _decision_.** ADR-0009 deleted `accept`/`reject`,
...
- **`COPILOT_ENABLED=true`.** Off by default (`apps/server/ortha.config.ts:194`,
  ADR-0005 §10). With it off, every route still authenticates and the _run_
...
- `@orthacms/identity-server` — `PermissionsService.forRole` is the _only_
...
| F12 | A throwing tool becomes a tool _error_; the run continues                                                                           | `run-engine.service.ts:781-787`                                       | ✅ E2E `copilot-chat.spec.ts:411`                                                                              |
...
| F47 | An `effect: 'apply'` tool's _execution_ records a proposal                                                                          | — **does not exist**                                                  | ❌ NONE → 🐞 BUG-copilot-server-02                                                                             |
...
| Step | Action                                                                                 | Expected result                                                                                                                                                                                                                                     |
| ---- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Script `DEFAULT_RUN_LIMITS.maxSteps + 1` turns, each asking for a _distinct_ tool call | The run ends with `done.stopReason = "max-steps"` after exactly `maxSteps` model calls                                                                                                                                                              |
...
- **EC-01 — `message` of exactly one space.** `❌ NONE`
  ...
  `UpdateConversationDto`, which trims _before_ validating
...
- **EC-20 — HTML/script in a skill's `instructions`.** `❌ NONE`
  ...
  holds `copilot:skills:manage`. Worth asserting the _documented_ behaviour so a
...
- **EC-42 — two "allow for this chat" answers in one turn.** `⚠️ PARTIAL`
  `copilot-proposals.spec.ts:328` covers a second _turn_; the two-calls-in-one-turn
...
- **EC-43 — a role revoked between the offer and the call.** `⚠️ PARTIAL`
  ...
  revokes mid-run to exercise the _fresh_ resolution at
...
- **EC-54 — an entry whose _title_ carries an injection, surfaced through a tool
...
What _is_ squarely in scope is **504 Authoring Tools**, because this is the code
...
whether it _prompts_ for accessibility information. An agent write path that
...
**What happens.** The copilot _can_ set alt text — `media_propose_alt_text`
...
nothing), or alt text that describes the _file_ rather than the content —
...
**What happens.** 504.3 requires an authoring tool to _prompt_ the author for
...
that returns a _tool error_ the model can fix, the same way
...
**WCAG 2.1 SC:** 4.1.3 Status Messages (AA) — _upstream cause_
...
`stopReason` is persisted on the assistant message, so a reopened thread _can_
...
| F5 persist-before-call             | `copilot-chat.spec.ts:290`                                                                 | the turn is on the transcript afterwards                                                                                                                                                                                                                       | ⚠️ PARTIAL — asserts the _result_, not the _ordering_; a provider failure is never induced                                                                                                         |
...
**Why it is wrong:** ADR-0009 §1b makes this route the _entire_ mitigation for
...
belongs to the workspace they named; nothing upstream proves a _conversation id_
...
(`:33-41`) reasons carefully about why it does not re-check _permissions_, and
...
/api/copilot/runs/<runId>/permission` with `{"callId":"…","decision":"once"}`
   and `X-Workspace-Id` naming _any_ workspace the viewer belongs to.
...
five-minute park. But `runId` is _served_ — `copilot_proposals.runId` is on
...
step: "It is still written _before_ the apply … A binder that wrote directly
...
The e2e that _would_ catch it exists as a fixture and is only ever asserted at
offer time.
...
- **The token ceiling overshoots by a whole model call**, because it is checked
  _before_ the call that will exceed it.
...
trail" for a change that has _already been made to content that still exists_.
...
But `fenceUntrusted` exists because ADR-0005 §8 chose _structural_ framing over
...
to a colleague plants attacker-chosen text in _their_ system prompt, running
with _their_ grants. That is the one shape where "the caller is the authority"
...
unscoped methods are safe _by call site_, not _by construction_.
...
- **Does the repeat guard leak information?** No — it runs _after_ authorization
