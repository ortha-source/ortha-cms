...
| Deterministic tests       | partial  | `server-e2e` now covers the whole loop over the fake provider — guards, the strict pipe, the stream, the tool loop, the capability profile, the content tools. `admin-e2e` now covers the **Agents view** end to end (`src/copilot/`): `page.route` _can_ fulfil an event-stream body — the whole body arrives in one read, so only progressive arrival is lost, not the transcript or its block order. Remaining gap: **the docked panel's own chrome** — tiling, the window cap, drag persistence, Escape. |
