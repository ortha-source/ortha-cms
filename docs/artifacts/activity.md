...
| The entry's Activity block | ENTRY_SIDEBAR_WIDGET_SLOT | `EntryActivityWidget`: the 6 most recent actions on the open record, order 60 — last in the rail, because history is context you go looking for rather than the reason you opened the editor | `content:read`, and it reads a different route; without the permission it renders `null` and makes no request |
