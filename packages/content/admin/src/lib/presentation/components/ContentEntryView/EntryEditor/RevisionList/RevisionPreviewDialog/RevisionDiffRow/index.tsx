 ...
 * renders the actual linked records (`RelationRefList`, resolved server-side); a
 * scalar renders its formatted value. An **unchanged** field shows a single
 * value (only revealed when the user expands the unchanged set); a **changed**
 * field shows a Current → Version {n} pair with a "Changed" badge — exactly what
 * restoring this version would put back.
 */
export function RevisionDiffRow({
    ...
    /** Resolved relation records (undefined for scalar fields). */
