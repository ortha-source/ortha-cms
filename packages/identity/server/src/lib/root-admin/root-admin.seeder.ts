...
import type { IdentityPluginConfig, IdentityRootAdminConfig } from '../types';
...
export class RootAdminSeeder implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap(): Promise<void> {
        ...
        const passwordHash = await this.resolveHash(rootAdmin);
    ...
    /** A pre-hashed value wins; otherwise bcrypt the plaintext password. */
    private async resolveHash(config: IdentityRootAdminConfig): Promise<string> {
        if (config.passwordHash) return config.passwordHash;
        if (config.password) return this.hashing.hashPassword(config.password);
        throw new MissingRootAdminPasswordError(config.email);
    }
}
